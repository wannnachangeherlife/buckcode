...
OBSIDIAN_VAULT_PATH=D:\path\to\your\notes\vault
```

**方式 B：自动读取（已配置）**
- 脚本会自动从 `automation/utils/notion_databases.json` 读取 Task Management ID
- 无需额外配置

**方式 C：使用默认路径**
- Obsidian Vault 默认路径：`./notes/vault`
...
```powershell
# 干运行模式（默认，不写入文件，仅预览）
python automation/sync/notion_to_obsidian_sync.py

# 实际同步模式（写入 Markdown 文件）
$env:DRY_RUN="false"; python automation/sync/notion_to_obsidian_sync.py
```
...
3. 操作：
   - 程序：`powershell.exe`
   - 参数：`-Command "cd D:\path\to\magicalgitzone; $env:DRY_RUN='false'; python automation/sync/notion_to_obsidian_sync.py"`
...
# 添加定时任务（每天 8:00）
0 8 * * * cd /path/to/magicalgitzone && DRY_RUN=false python automation/sync/notion_to_obsidian_sync.py
```
...
创建示例任务用于测试：
```powershell
python automation/utils/create_sample_task.py
```
...
- A: 确保 `automation/utils/notion_databases.json` 存在且包含 Task Management 条目
